...
prob, alg, NSTEPS = _solve_heat1d(NSTEPS=1, δ=δ , file=file, model=StepIntersect(Forward()))
n = statedim(prob);
...
println("Flowpipe solution with gradient directions (LGG)")
@time grad_lgg = solve(prob, alg=LGG09(δ=δ, template=LGG_grad_dirs, approx_model=Forward(setops=:box)), NSTEPS=k);

NCURVAS = 1_000
println("Trajectories solution (Euler's method, $NCURVAS Trajectories)")
